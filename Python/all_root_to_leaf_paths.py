# Print all root-to-leaf paths in a BST using concept of Stacks
...
# Using Stacks to print all root-leaf paths in a BST
...
class Solution:
  ...
  def all_root_to_leaf(self, root:Node):
    # pre-order traversal
    if root is None:
      return 
    ...
    if root.left is None and root.right is None:
      ...
      self.node_stack.pop() 
      return 
    self.all_root_to_leaf(root.left)
    self.all_root_to_leaf(root.right)
    self.node_stack.pop()
